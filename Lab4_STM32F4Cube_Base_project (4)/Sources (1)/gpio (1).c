...
int SegmentEncoder(int toDecode, int * segArr, int digit){
	...
	if(toDecode<0){
		...
		setSegmentSelectLines(segArr);
		return isNegative;
}
void setSegmentSelectLines(int * segmentArray){
	for(int i = 0 ; i < 7 ; i++){
		if(segmentArray[i]) HAL_GPIO_WritePin(GPIOB, segments[i], GPIO_PIN_SET);
		else HAL_GPIO_WritePin(GPIOB,segments[i], GPIO_PIN_RESET);
	}
}
void resetDigitSelectLines(void){
	HAL_GPIO_WritePin(GPIOC, DIGIT_CONTROL_1, GPIO_PIN_RESET);
	HAL_GPIO_WritePin(GPIOC, DIGIT_CONTROL_2, GPIO_PIN_RESET);
	HAL_GPIO_WritePin(GPIOC, DIGIT_CONTROL_3, GPIO_PIN_RESET);
	HAL_GPIO_WritePin(GPIOC, DIGIT_CONTROL_4, GPIO_PIN_RESET);
}
void setDigitSelectLines(int digit){
	...
	{
		case 0:
			HAL_GPIO_WritePin(GPIOC, DIGIT_CONTROL_1, GPIO_PIN_SET);
			HAL_GPIO_WritePin(GPIOB, SEG7, GPIO_PIN_RESET);		
			break;
		case 1:
			HAL_GPIO_WritePin(GPIOC, DIGIT_CONTROL_2, GPIO_PIN_SET); 
			HAL_GPIO_WritePin(GPIOB, SEG7, GPIO_PIN_RESET);
			break;
		case 2:
			HAL_GPIO_WritePin(GPIOC, DIGIT_CONTROL_3, GPIO_PIN_SET);
			HAL_GPIO_WritePin(GPIOB, SEG7, GPIO_PIN_SET);		
			break;
		case 3:
			HAL_GPIO_WritePin(GPIOC, DIGIT_CONTROL_4, GPIO_PIN_SET); 
			HAL_GPIO_WritePin(GPIOB, SEG7, GPIO_PIN_RESET);
			break;
	}
}
